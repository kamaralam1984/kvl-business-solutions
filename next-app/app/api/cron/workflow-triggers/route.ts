...
export async function GET(req: Request) {
  ...
  try {
    ...
    await logCronRun('workflow-triggers', 'success', `lead_inactive_3d: ${results.lead_inactive_3d}, cart_abandoned: ${results.cart_abandoned}`);
    return NextResponse.json({ ok: true, triggered: results, timestamp: now.toISOString() });
  } catch (e: any) {
    await logCronRun('workflow-triggers', 'error', 'Run failed', e.message);
