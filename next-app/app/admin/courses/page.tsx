  ...
  return (
    <div className="max-w-4xl">
      <div className="flex justify-between items-end mb-4">
        <h1 className="text-2xl font-extrabold" style={{ color: 'rgb(var(--text))' }}>Courses ({courses.length})</h1>
      ...
      <div className="card-base overflow-hidden kpi-enter" style={{ background: 'linear-gradient(135deg, rgb(var(--bg-2)) 0%, rgb(var(--bg-3)) 100%)', border: '1px solid rgba(var(--border) / 0.06)' }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {courses.map(c => (
              <tr key={c._id} className="transition-colors duration-150 hover:bg-surface/[0.025]" style={{ borderBottom: '1px solid rgba(var(--border) / 0.05)' }}>
                <td className="p-3 font-semibold" style={{ color: 'rgb(var(--text))' }}>{c.title}</td>
                <td className="p-3 font-mono text-xs" style={{ color: 'rgba(var(--text) / 0.4)' }}>{c.slug}</td>
                <td className="p-3" style={{ color: 'rgba(var(--text) / 0.6)' }}>{c.level}</td>
      ...
      {editing && (
        ...
        >
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-extrabold" style={{ color: 'rgb(var(--text))' }}>{isNew ? 'New Course' : 'Edit Course'}</h2>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <select className="form-control" value={editing.level} onChange={e => setEditing({ ...editing, level: e.target.value as Course['level'] })}>
                  <option value="Beginner" style={{ background: 'rgb(var(--bg-2))', color: 'rgb(var(--text))' }}>Beginner</option>
                  <option value="Intermediate" style={{ background: 'rgb(var(--bg-2))', color: 'rgb(var(--text))' }}>Intermediate</option>
                  <option value="Advanced" style={{ background: 'rgb(var(--bg-2))', color: 'rgb(var(--text))' }}>Advanced</option>
                </select>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-[11px] font-semibold uppercase" style={{ color: 'rgba(var(--text) / 0.4)' }}>Lessons</label>
