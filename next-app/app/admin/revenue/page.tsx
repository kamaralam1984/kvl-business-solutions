...
export default function RevenueDashboardPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div>
        <h1 className="font-display font-black text-[1.8rem] tracking-tight leading-none" style={{ color: '#f0ede6' }}>Revenue Dashboard</h1>
      ...
      {loading ? (
        ...
        <>
          <div className="grid sm:grid-cols-3 gap-4 stagger-children">
            {kpis.map(k => (
              <div key={k.label} className="admin-card-hover kpi-enter rounded-2xl p-5" style={CARD}>
                ...
                <div className="text-xl font-extrabold" style={{ color: '#f0ede6' }}>{k.val}</div>
              </div>
            ))}
          </div>

          <div className="kpi-enter rounded-2xl p-5" style={CARD}>
            <h2 className="text-[13px] font-bold mb-4" style={{ color: '#f0ede6' }}>Daily Visitors — Last 30 Days</h2>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="kpi-enter rounded-2xl p-5" style={CARD}>
              <h2 className="text-[13px] font-bold mb-4" style={{ color: '#f0ede6' }}>Best Lead Sources (30d)</h2>
              <div className="space-y-2">
                {data.topSources.length === 0 && <p className="text-[12px]" style={{ color: 'rgba(255,255,255,0.3)' }}>No leads yet.</p>}
                {data.topSources.map((s: any) => (
                  <div key={s.source} className="flex items-center justify-between text-[12px]">
                    <span style={{ color: 'rgba(255,255,255,0.6)' }} className="truncate mr-2">{s.source}</span>
                    <span className="font-semibold shrink-0" style={{ color: '#f0ede6' }}>{s.count}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="kpi-enter rounded-2xl p-5" style={CARD}>
              <h2 className="text-[13px] font-bold mb-4" style={{ color: '#f0ede6' }}>Top Requested Services (30d)</h2>
              <p className="text-[10px] mb-3" style={{ color: 'rgba(255,255,255,0.3)' }}>Proxy for industry interest — Lead has no dedicated industry field.</p>
              <div className="space-y-2">
                {data.topServices.length === 0 && <p className="text-[12px]" style={{ color: 'rgba(255,255,255,0.3)' }}>No leads yet.</p>}
                {data.topServices.map((s: any) => (
                  <div key={s.service} className="flex items-center justify-between text-[12px]">
                    <span style={{ color: 'rgba(255,255,255,0.6)' }} className="truncate mr-2">{s.service}</span>
                    <span className="font-semibold shrink-0" style={{ color: '#f0ede6' }}>{s.count}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="kpi-enter rounded-2xl p-5" style={CARD}>
              <h2 className="text-[13px] font-bold mb-4" style={{ color: '#f0ede6' }}>Top Landing Pages for Leads (30d)</h2>
              <div className="space-y-2">
                {data.topLandingPagesForLeads.length === 0 && <p className="text-[12px]" style={{ color: 'rgba(255,255,255,0.3)' }}>No lead events tracked yet.</p>}
                {data.topLandingPagesForLeads.map((p: any) => (
                  <div key={p.path} className="flex items-center justify-between text-[12px]">
                    <span style={{ color: 'rgba(255,255,255,0.6)' }} className="truncate mr-2">{p.path}</span>
                    <span className="font-semibold shrink-0" style={{ color: '#f0ede6' }}>{p.count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="kpi-enter rounded-2xl p-5 flex items-start gap-3" style={{ ...CARD, borderColor: 'rgba(234,179,8,0.25)' }}>
            ...
            <div>
              <h3 className="text-[13px] font-bold" style={{ color: '#f0ede6' }}>Campaign ROI — not available yet</h3>
              <p className="text-[12px] mt-1" style={{ color: 'rgba(255,255,255,0.45)' }}>{data.campaignRoi.reason}</p>
            </div>
          </div>
        </>
      )}
