...
export default function CampaignDetailPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <button onClick={() => router.push('/admin/outreach')} className="text-[12px] flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.5)' }}>
        <ArrowLeft className="w-3.5 h-3.5" /> All campaigns
      </button>

      <div>
        <h1 className="font-display font-black text-[1.6rem] tracking-tight" style={{ color: '#f0ede6' }}>{campaign.name}</h1>
        <div className="flex gap-4 text-[12.5px] mt-2" style={{ color: 'rgba(255,255,255,0.4)' }}>
      ...
      {bulkOpen && (
        <div className="rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, #0f0f12 0%, #111114 100%)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <p className="text-[12px] mb-2" style={{ color: 'rgba(255,255,255,0.4)' }}>One per line: <code>Name, email, company</code> — email and company optional.</p>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f0f12 0%, #111114 100%)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <tbody>
            {prospects.map(p => (
              <tr key={p._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <td className="p-3">
                  <div className="font-semibold" style={{ color: '#f0ede6' }}>{p.name}</div>
                  <div className="text-[11px]" style={{ color: 'rgba(255,255,255,0.4)' }}>{p.company}{p.email ? ` · ${p.email}` : ''}</div>
                </td>
                <td className="p-3">
                  <select
                  ...
                  >
                    {PROSPECT_STATUSES.map(s => <option key={s} value={s} style={{ background: '#0f0f12' }}>{s.replace('_', ' ')}</option>)}
                  </select>
                </td>
                <td className="p-3">
                  <button onClick={() => setOpenDraftId(openDraftId === p._id ? null : p._id)} className="text-[11px] flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.5)' }}>
                    <ClipboardList className="w-3.5 h-3.5" /> {openDraftId === p._id ? 'Hide' : 'Generate draft'}
                  </button>
                </td>
                <td className="p-3 text-right whitespace-nowrap">
                  {!p.convertedDealId ? (
                    <button onClick={() => convert(p)} title="Convert to CRM deal" className="p-1 inline-block" style={{ color: 'rgba(255,255,255,0.4)' }}><HeartHandshake className="w-4 h-4" /></button>
                  ) : (
                    <span className="text-[10px] font-bold text-green-400 mr-1">In CRM</span>
                  )}
                  <button onClick={() => del(p)} className="p-1 inline-block" style={{ color: 'rgba(255,255,255,0.4)' }}><Trash2 className="w-4 h-4" /></button>
                </td>
              </tr>
            ))}
            {prospects.length === 0 && <tr><td colSpan={4} className="p-8 text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>No prospects yet — add some above.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {prospects.filter(p => p._id === openDraftId).map(p => {
        ...
        return (
          <div key={p._id} className="rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, #0f0f12 0%, #111114 100%)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <h3 className="font-bold mb-2" style={{ color: '#f0ede6' }}>Draft for {p.name}</h3>
            {campaign.channel === 'email' && subject && (
              <p className="text-[12px] mb-2" style={{ color: 'rgba(255,255,255,0.5)' }}><b>Subject:</b> {subject}</p>
            )}
            <pre className="whitespace-pre-wrap text-[13px] p-3 rounded-lg mb-3" style={{ background: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.7)' }}>{body}</pre>
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => { navigator.clipboard.writeText(body); }} className="btn text-xs" style={{ background: 'rgba(255,255,255,0.08)', color: '#f0ede6' }}>
            ...
            <p className="text-[10.5px] mt-2" style={{ color: 'rgba(255,255,255,0.3)' }}>
