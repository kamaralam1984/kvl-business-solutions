        ...
        <div className="container grid lg:grid-cols-2 gap-10">
          ...
          <form onSubmit={submit} className="card-base p-7">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-3">
              <input className="form-control" placeholder="Name *" required value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
            ...
            <select className="form-control mb-3" value={form.product} onChange={e => setForm({...form, product: e.target.value})}>
              {['ERP Software','Website','GPS Tracking','HMS','School ERP','Other'].map(o => <option key={o}>{o}</option>)}
            </select>
            <select className="form-control mb-3" value={form.priority} onChange={e => setForm({...form, priority: e.target.value})}>
              {['Low','Medium','High','Critical'].map(o => <option key={o}>{o}</option>)}
            </select>
            <textarea className="form-control mb-3 min-h-[120px]" placeholder="Describe your issue *" required value={form.description} onChange={e => setForm({...form, description: e.target.value})} />
            {session?.user && (
              <div className="mb-4">
