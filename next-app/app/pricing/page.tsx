      ...
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden" style={{ background: '#0a0a0a' }}>
        ...
        <div className="relative z-10 container text-center py-28">
          ...
          <motion.p
            ...
            style={{ color: '#888' }}
          >
            No hidden fees. No surprises. 30-day money-back on everything.
          </motion.p>
          ...
          >
            <span className="text-sm" style={{ color: isAnnual ? '#888' : '#f5f5f0' }}>Monthly</span>
            <button
              onClick={() => setIsAnnual(!isAnnual)}
              className="relative w-12 h-6 rounded-full transition-colors duration-300"
              style={{ background: isAnnual ? '#c8a96e' : 'rgba(255,255,255,0.1)' }}
            >
              <span
            ...
            <span className="text-sm" style={{ color: isAnnual ? '#f5f5f0' : '#888' }}>
      ...
      <section className="section" style={{ background: '#0a0a0a' }}>
        <div className="container">
          <FadeIn className="text-center mb-4">
            <h2 className="text-3xl font-extrabold" style={{ color: '#f5f5f0', fontFamily: 'Poppins, sans-serif' }}>Compare plans</h2>
            <p className="mt-2 text-sm" style={{ color: '#888' }}>Bundle multiple products and save up to 30%.</p>
          </FadeIn>
              ...
              return (
                <FadeIn key={tier.name} delay={idx * 0.1}>
                  ...
                  >
                    {tier.tag && (
                      <div
                        className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full text-[11px] font-bold"
                        style={{
                          background: isPopular ? 'rgba(200,169,110,0.15)' : 'rgba(255,255,255,0.07)',
                          border: isPopular ? '1px solid rgba(200,169,110,0.4)' : '1px solid rgba(255,255,255,0.15)',
                          color: isPopular ? '#c8a96e' : '#f5f5f0',
                        }}
                      >
                        {tier.tag}
                      </div>
                    )}
                    <h3 className="text-2xl font-extrabold mt-2" style={{ color: '#f5f5f0', fontFamily: 'Poppins, sans-serif' }}>{tier.name}</h3>
                    <p className="text-xs mt-1 mb-5" style={{ color: '#888' }}>{tier.desc}</p>
                    <div className="mb-6">
                      {discount > 0 && (
                        <div className="text-sm line-through" style={{ color: 'rgba(255,255,255,0.25)' }}>{formatINR(Math.round(total * (1 - annualDiscount)))}</div>
                      )}
                      <div className="text-4xl font-extrabold" style={{ color: '#f5f5f0', fontFamily: 'Poppins, sans-serif' }}>
                        {formatINR(final)}
                        <span className="text-base font-normal ml-1" style={{ color: '#888' }}>/{isAnnual ? 'year' : 'mo'}</span>
                      </div>
                      {discount > 0 && (
                        <div className="text-xs font-semibold mt-1" style={{ color: '#4ade80' }}>Save {Math.round(discount * 100)}% as a bundle</div>
                      )}
                    </div>
                    <div className="space-y-2 text-sm mb-5 pb-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                      <div className="text-xs uppercase font-bold mb-3 tracking-wider" style={{ color: '#888' }}>Includes</div>
                      {products.map(p => (
                    ...
                    <div className="space-y-2 text-sm mb-6 flex-1">
                      {tier.extras.map(e => (
                        <div key={e} className="flex gap-2 items-center" style={{ color: '#888' }}>
      ...
      <section className="section" style={{ background: '#111111' }}>
        <div className="container">
          <FadeIn className="text-center mb-12">
            ...
            <h2 className="text-3xl font-extrabold my-3" style={{ color: '#f5f5f0', fontFamily: 'Poppins, sans-serif' }}>All plans include</h2>
          </FadeIn>
            ...
            {universalFeatures.map((f, i) => (
              <FadeIn key={f} delay={i * 0.04}>
                <div className="card-premium flex items-center gap-3 px-4 py-3">
                  <Check className="w-4 h-4 shrink-0" style={{ color: '#c8a96e' }} />
                  <span className="text-sm" style={{ color: '#f5f5f0' }}>{f}</span>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="section" style={{ background: '#0a0a0a' }}>
        <div className="container">
          <FadeIn className="text-center mb-10">
            <h2 className="text-3xl font-extrabold" style={{ color: '#f5f5f0', fontFamily: 'Poppins, sans-serif' }}>Individual products</h2>
            <p className="mt-2 text-sm" style={{ color: '#888' }}>Buy à la carte. All prices in INR, GST extra.</p>
          </FadeIn>
          <FadeIn delay={0.1}>
            <div className="card-premium overflow-x-auto">
              <table className="w-full text-sm min-w-[720px]">
                <thead style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                  <tr className="text-left text-xs uppercase tracking-wider" style={{ color: '#888' }}>
                ...
                <tbody>
                  {softwareProducts.map(p => {
                    ...
                    return (
                      <tr key={p.slug} className="transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        <td className="p-4">
                          <div className="font-bold" style={{ color: '#f5f5f0' }}>{p.name}</div>
                          <div className="text-xs mt-0.5" style={{ color: '#888' }}>{p.category}</div>
                          <ul className="mt-1.5 space-y-0.5">
                            {p.features?.slice(0, 2).map((f: string) => (
                              <li key={f} className="flex gap-1 items-center text-xs" style={{ color: '#888' }}>
                        ...
                        <td className="p-4 text-right">
                          <div className="font-bold text-base" style={{ color: '#f5f5f0' }}>{formatINR(p.price)}</div>
                          <div className="text-[10px]" style={{ color: '#888' }}>{p.unit}</div>
                        </td>
                        <td className="p-4 text-right">
                          <div className="font-bold" style={{ color: '#888' }}>{formatINR(onPrem)}</div>
                          <div className="text-[10px]" style={{ color: '#888' }}>{p.unit}</div>
                        </td>
                        <td className="p-4 text-right">
      ...
      <section className="section" style={{ background: '#111111' }}>
        <div className="container max-w-3xl">
          <FadeIn className="text-center mb-12">
            ...
            <h2 className="text-3xl font-extrabold my-3" style={{ color: '#f5f5f0', fontFamily: 'Poppins, sans-serif' }}>
            ...
            {pricingFaqs.map((faq, i) => (
              <FadeIn key={i} delay={i * 0.06}>
                <div className="card-premium overflow-hidden">
                  ...
                  >
                    <span className="font-semibold text-sm" style={{ color: '#f5f5f0' }}>{faq.q}</span>
                    <span
                  ...
                  >
                    <p className="px-5 pb-5 text-sm leading-relaxed" style={{ color: '#888' }}>{faq.a}</p>
                  </motion.div>
                </div>
              </FadeIn>
            ))}
            ...
            {[
              ...
              <FadeIn key={t.title} delay={i * 0.1}>
                <div className="card-premium p-6 text-center">
                  ...
                  <h4 className="font-bold mb-2 text-sm" style={{ color: '#f5f5f0' }}>{t.title}</h4>
                  <p className="text-sm" style={{ color: '#888' }}>{t.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="section" style={{ background: '#111111' }}>
        <div className="relative z-10 container text-center">
          <FadeIn>
            ...
            <p className="mb-7 max-w-xl mx-auto text-sm" style={{ color: '#888' }}>
              Our experts will recommend the perfect bundle for your business size and industry. Free 30-minute consultation.
            </p>
